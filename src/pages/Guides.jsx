  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <span className="text-xs font-mono text-text-muted flex-shrink-0">{guide.readTime}</span>
      </div>
...
function ExplainerRow({ guide, index }) {
  return (
    <motion.div
      ...
      onClick={() => alert(`Opening explainer: ${guide.title}`)}
    >
      <div
      ...
      <div className="pt-16">
        ...
        <section className="relative py-20 overflow-hidden">
          <div
            ...
            style={{ background: 'radial-gradient(circle, #F5C842, transparent 70%)', top: '-10%', right: '15%' }}
          />
          <div className="container-wide relative z-10">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <p className="text-xs font-mono text-accent-gold uppercase tracking-widest mb-3">Learn</p>
        ...
        <div className="container-wide pb-20 space-y-16">
          ...
          <section>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="font-heading text-3xl tracking-heading text-text-primary">BUYER'S GUIDES</h2>
                <p className="text-sm text-text-muted">Essential reading before you spend five figures on a robot</p>
              </div>
            </div>
          ...
          <section>
            ...
            >
              ...
              >
                <span className="text-2xl">🔬</span>
                <div>
                  <h2 className="font-heading text-3xl tracking-heading text-text-primary">EXPLAINERS</h2>
                  <p className="text-sm text-text-muted">Technical concepts explained clearly for non-engineers</p>
                </div>
              </div>
              <div className="px-6">
                {explainers.map((guide, i) => (
                  <ExplainerRow key={guide.id} guide={guide} index={i} />
                ))}
              </div>
            </div>
          </section>
          ...
          <section>
            <div className="flex items-center gap-3 mb-6">
              <span className="text-2xl">🌐</span>
              <div>
                <h2 className="font-heading text-3xl tracking-heading text-text-primary">INDUSTRY DEEP DIVES</h2>
                <p className="text-sm text-text-muted">Longform analysis of the forces shaping robotics</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {deepDives.map((guide, i) => (
                <motion.div
                  ...
                  onClick={() => alert(`Opening deep dive: ${guide.title}`)}
                  onMouseEnter={(e) => {
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <span className="text-xs font-mono text-text-muted">{guide.readTime}</span>
                  </div>
